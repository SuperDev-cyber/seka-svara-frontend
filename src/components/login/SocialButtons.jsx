...
const SocialButtons = () => {
    ...
    const handleSafeAuthWallet = async () => {
        try {
            ...
            // Register/Login with backend using wallet address
            try {
                // Try to login first
                const loginResponse = await apiService.post('/auth/login', {
            ...
            console.error('SafeAuth Wallet login error:', err);
            if (window.showToast) {
                window.showToast(err.message || 'Wallet sign-in failed', 'error', 5000);
            }
        } finally {
            setLoading(false);
        }
