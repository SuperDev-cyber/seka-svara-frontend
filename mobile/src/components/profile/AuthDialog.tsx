...
import { connectTo, WALLET_CONNECTORS, AUTH_CONNECTION } from '@/lib/web3';
...
export function AuthDialog({ open, onClose }: AuthDialogProps) {
  const { signInWithOtp, verifyOtpAndSetPassword, signIn } = useAuth();
  ...
  const handleVerifyAndRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.message || 'Tekshirish xatosi');
    } finally {
      setLoading(false);
    }
  };

  const handleWeb3AuthLogin = async (provider: AUTH_CONNECTION) => {
    setLoading(true);
    try {
      const address = await connectTo(WALLET_CONNECTORS.AUTH, {
        authConnection: provider,
        authConnectionId: `w3a-${provider}-demo`,
      });
      
      // Connect wallet after Web3Auth authentication
      await connectWallet();
      
      toast.success('Muvaffaqiyatli ulandi!');
      onClose();
    } catch (error: any) {
      toast.error(error.message || 'Ulanishda xatolik');
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {mode === 'login' && (
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-3">
              <Button
                type="button"
                onClick={() => handleWeb3AuthLogin(AUTH_CONNECTION.GOOGLE)}
                disabled={loading}
        ...
        {mode === 'register' && (
          <form onSubmit={handleStartRegister} className="space-y-4">
            ...
            <Button type="submit" disabled={loading} className="w-full gradient-primary">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'OTP yuborish'}
            </Button>
